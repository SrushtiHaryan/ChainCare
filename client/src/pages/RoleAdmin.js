...
function RoleAdmin(props) {
  ...
  const [patientRole, setPatientRole] = React.useState("");
  const navItem = [];
  ...
  const handleAddPatientRole = async () => {
   ...
   localStorage.setItem("cRole", patientRole);
    await supplyChainContract.methods.addPatientRole(patientRole).send({ from: accounts[0], gas:100000 })
    .then(console.log);
