...
export default function ReceivePatient(props) {
  ...
  const navItem = [
    ["Purchase Product", "/Patient/buy"],
    ["Receive Product", "/Patient/receive"],
    ["Your Products", "/Patient/allReceived"],
  ];
  ...
  const handleReceiveButton = async (id) => {
    try{
      ...
      .receiveByPatient(parseInt(id))
      .send({ from: roles.patient, gas: 1000000 })
  ...
  return (
    <div classname={classes.pageWrap}>
      <Navbar pageTitle={"Patient"} navItems={navItem}>
        {loading ? (
          <Loader />
        ) : (
          <>
