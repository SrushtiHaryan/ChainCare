...
contract SupplyChain {
    ...
    function hasPatientRole(address _account) public view returns (bool) {
        require(_account != address(0));
        return roles[_account].Patient;
    }

    function addPatientRole(address _account) public {
        require(_account != address(0));
        require(!hasDeliveryHubRole(_account));

        roles[_account].Patient = true;
    }
    ...
    modifier purchasedByPatient(uint256 _uid) {
        require(
            products[_uid].productState == Structure.State.PurchasedByPatient
        );
        _;
    }
    ...
    modifier receivedByPatient(uint256 _uid) {
        require(
            products[_uid].productState == Structure.State.ReceivedByPatient
        );
        _;
    }
    ...
    ///@dev STEP 5 : Purchase of a product at third party by Patient.
    function purchaseByPatient(uint256 _uid)
        public
        receivedByThirdParty(_uid)
    {
        require(hasPatientRole(msg.sender));
        products[_uid].patient = msg.sender;
        products[_uid].productState = Structure.State.PurchasedByPatient;
        productHistory[_uid].history.push(products[_uid]);

        emit PurchasedByPatient(_uid);
    }

    ///@dev STEP 7 : Shipping of product by third party purchased by patient.
    ...
    ///@dev STEP 8 : Receiveing of product by delivery hub purchased by patient.
    ...
    ///@dev STEP 9 : Shipping of product by delivery hub purchased by patient.
    ...
    ///@dev STEP 10 : Shipping of product by delivery hub purchased by patient.
    function receiveByPatient(uint256 _uid)
        public
        shippedByDeliveryHub(_uid)
        verifyAddress(products[_uid].patient)
    {
        require(hasPatientRole(msg.sender));
        products[_uid].owner = msg.sender;
        products[_uid].productState = Structure.State.ReceivedByPatient;
